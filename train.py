...
def build_rnn(cell_type=None, isBidirectional=False, isBatchNorm=False, isNoise=False):
    ...
    if isNoise:
        gaussian_input_main_data = k.layers.GaussianNoise(0.5, name='gaussian_main')(input_main_data)
        cell_main = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='cell_main_1'), isBidirectional)(gaussian_input_main_data), isBatchNorm)
    else:
        cell_main = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='cell_main_1'), isBidirectional)(input_main_data), isBatchNorm)

    aux_output = batchNorm(k.layers.TimeDistributed(k.layers.Dense(1, name='aux_output'))(cell_main), isBatchNorm)

    if isNoise:
        gaussian_input_aux_data = k.layers.GaussianNoise(0.5)(input_aux_data)
        cell_aux = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='cell_aux_1'), isBidirectional)(gaussian_input_aux_data), isBatchNorm)
    else:
        cell_aux = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='cell_aux_1'), isBidirectional)(input_aux_data), isBatchNorm)

    cell = k.layers.concatenate([cell_main, cell_aux], name='concatenate')
    cell = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='encode_layer'), isBidirectional)(cell), isBatchNorm)

    flatten = k.layers.Flatten(name='flatten')(cell)
    encode_output = batchNorm(k.layers.Dense(512, name='dnn_all_1')(flatten), isBatchNorm)
    encode_output = k.layers.RepeatVector(seq_size, name='encode_output')(encode_output)

    cell = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='decode_layer_1'), isBidirectional)(encode_output), isBatchNorm)
    cell = batchNorm(bidirectionalCell(rnn_cell(128, return_sequences=True, name='decode_layer_2'), isBidirectional)(cell), isBatchNorm)
    decode_output = batchNorm(k.layers.TimeDistributed(k.layers.Dense(1, name='decode_output'))(cell), isBatchNorm)

    model = k.models.Model([input_main_data, input_aux_data], [decode_output, aux_output])
    model.summary()

    model.compile(optimizer=k.optimizers.Adam(lr=lr), loss=loss, metrics=[k.metrics.mae], loss_weights=[1, 0.2])
    return model


def train():
    model = build_rnn(cell_type=None, isBatchNorm=True, isBidirectional=True, isNoise=False)
    model.fit(x=[x_main[0: (x_main.shape[0] - 1)], x_aux[0: (x_aux.shape[0] - 1)]], y=[y[1: y.shape[0]], y[1: y.shape[0]]], batch_size=batch_size, epochs=epochs,
              callbacks=[
                  k.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.7, patience=30, verbose=1, cooldown=o),
                  k.callbacks.ModelCheckpoint(filepath='./model/bmw/checkpoint/model-{epoch:02d}-{val_loss:.2f}.hdf5',
                                              verbose=1, period=500, save_best_only=True, mode='min'),
...
def getData():
    ...
    for dmap in result_datas:
        ...
        x_aux.append([dmap['ExistenceProbability'], dmap['IndictatorLeft'], dmap['IndicatorRight'], dmap['TypeProbabilityCar'],
                      dmap['TypeProbabilityTruck'], dmap['TypeProbabilityBike'], dmap['TypeProbabilityStationary'], dmap['ObjectWidth'],
                      dmap['ObjectHeight'], dmap['ObjectLength'], dmap['PositionAbsX'], dmap['PositionAbsY'], dmap['PositionAbsZ'],
